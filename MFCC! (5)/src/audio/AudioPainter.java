...
//			System.out.println(curWeiBeg + " " + prevWeiBeg+"!!!!!!!");
...
//					System.out.println("speech");
            ...
            if (diff < -5) {
                ...
                if (voiceDecision == SLOW_RELEASE || voiceDecision == SPEECH) {
                    slowCounter++;
//					System.out.println("SLOOOW: " + (3 - slowCounter) + " left");
                }
                if (slowCounter > 3) {
                    slowCounter = 0;
                    decision = NO_SPEECH;
//					System.out.println("no speech");
    ...
    private static final Color[] colors = {
            ...
            /* 2 */,
            new Color(0, 255, 0, useWindowFunction ? 128 : drL == 1 ? 64 : 128) // Fo픲rier
            /* 3 */,
    ...
    static void refreshColors() {
        ...
        colors[2] = new Color(0, 255, 0, useWindowFunction ? 128: drL == 1 ? 64 : 128);
        colors[3] = new Color(255, 0, 0, useWindowFunction ? 128: drL == 1 ? 64 : 128);
    }
...
//			bg = Color.black;
    ...
    void drawBuffer(byte[] a) {
        ...
        if (buffer.length >= w) {
            g.setColor(colors[1]);
            for (int i = 0; i < lim; i++) {
                g.drawLine(approximateBuffer[i], buffer[i] + h / 2, approximateBuffer[i + drL], buffer[i + drL] + h / 2);
            }
        } else {
            for (int i = 0; i < lim; i++) {
                g.drawLine(approximateBuffer[i], 10 * buffer[i] + h / 2, approximateBuffer[i + drL], buffer[i + drL] + h / 2);
            }
        }
    }
    ...
    void drawMFCC(byte[] a) {
		...
		for (int j = 0; j < mfcc.length / 3; j++) {
			...
			g.drawLine(i, (int) -mfcc[j] * mfccScaler + h / 2, i + 10 * drL,(int) -mfcc[j + drL] * mfccScaler + h / 2);
			i += 10;
		}
    }
}
